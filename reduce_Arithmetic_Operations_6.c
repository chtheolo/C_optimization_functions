...
int main(int argc, char* argv[])
{
	...
	for (i=1; i<SIZE-1; i+=1 ) {
		for (j=1; j<SIZE-1; j+=1) {
			...
			res = input[(i-1)*SIZE + j -1] * (-1) + input[(i-1)*SIZE + j + 1] + input[i *SIZE + j -1] *(-2) + input[i * SIZE + j + 1] * 2 + input[(i+1)*SIZE + j -1] *(-1) + input[(i+1)*SIZE + j + 1];
			
			/*res += input[(i -1)*SIZE + j -1] * (-1);
			res += input[(i -1)*SIZE + j ] * 0;
			res += input[(i -1)*SIZE + j + 1] * 1;

			res += input[(i )*SIZE + j + -1] * (-2);
			res += input[(i )*SIZE + j ] * 0;
			res += input[(i )*SIZE + j + 1] * 2;
			...
			res = input[(i-1)*SIZE + j -1] + input[(i-1)*SIZE + j] *2 + input[(i-1)*SIZE + j +1] + input[(i+1)*SIZE + j-1]*(-1) + input[(i+1)*SIZE + j]*(-2) + input[(i+1)*SIZE + j + 1]*(-1);
			/*res += input[(i -1)*SIZE + j + -1] * 1;
			res += input[(i -1)*SIZE + j ] * 2;
			res += input[(i -1)*SIZE + j + 1] * 1;

			res += input[(i )*SIZE + j + -1] * 0;
			res += input[(i )*SIZE + j ] * 0;
			res += input[(i )*SIZE + j + 1] * 0;

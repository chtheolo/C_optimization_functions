...
int main(int argc, char* argv[])
{
	...
	for (i=1; i<SIZE-1; i+=1 ) {
		for (j=1; j<SIZE-1; j+=1) {
			...
			for (k = -1; k <= 1; k++) {
				for (h = -1; h <= 1; h++) {
					res += input[(i + k)*SIZE + j + h] * horiz_operator[k+1][h+1];
				}
			}

			/*i replace the pow fuction by the multiple operation because its faster*/
			p1 = res*res;

			res = 0;
			for (k = -1; k <= 1; k++) {
				for (h = -1; h <= 1; h++) {
					res += input[(i + k)*SIZE + j + h] * vert_operator[k+1][h+1];
				}
			}
	...
	for (i=1; i<SIZE-1; i++) {
		for ( j=1; j<SIZE-1; j++ ) {
			t = (output[i*SIZE+j] - golden[i*SIZE+j]) *(output[i*SIZE+j] - golden[i*SIZE+j]); /*raplece and here the pow function by a multiple operation*/
			PSNR += t;
		}
	}
